...
class IMA_params:
    ...
    Establish parameter class for IMA
            ...
            Parameters:
                    rank (int): target number of features in final matrices
                    u_bounds (np.array): array, length 2, of upper bounds for integers
                    l_bounds (np.array): array, length 2, of lower bounds for integers
                    maxiter (int): maximum number of iterations, default=1000000.0
                    stop_crit (int): criterion of differences for stopping, default=0.0001
...
def _IMA_Update_Factor(M, coeff, mkrp, mode, lambda_, params):
    '''
    Internal function to update U, V, and Lambda each iteration

            Parameters:
                    M (np.array): matrix to be updated (either U or V)
                    coeff (np.array): matrix used in updating algorithm
                    mkrp (np.array): matrix used in updating algorithm
                    mode (int): whether updating U or V
                    lambda_ (np.array): matrix of scaling factors
                    params (int): parameter object
...
def _IMA_Compute_Init_Scaled(h, l_bound, u_bound):
    '''
    Internal function to rescale if max val is above upper bound

            Parameters:
                    h (np.array): matrix to be rescaled
                    l_bound (int): lower bound
                    u_bound (int): upper bound
...
def _IMA_init(X, params):
    '''
    Internal function to initialize U, V, and Lambda

            Parameters:
                    X (np.array): observed data matrix
                    params (int): parameter object
